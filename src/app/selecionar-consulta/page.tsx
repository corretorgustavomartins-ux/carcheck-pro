...
const PLANOS = [
  {
    ...
    items: [
      ...
      '🚫 Restrições (judicial, administrativa)',
      '🚗 Roubo e furto',
      '💰 Preço Justo por IA',
      '📄 Relatório em PDF',
    ],
    ...
    // Custo da API: ~R$12,90 (plano Prata)
    // Margem: ~R$3,00
  },
  {
    id: 'completo',
    ...
    nao_inclui: [],
    // Custo da API: ~R$19,90 (plano Ouro)
    // Margem: ~R$16,00
  },
]
    ...
    <div style={{
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '0 20px', animation: 'fadeUp 0.4s ease' }}>
              ...
              >
                ...
                {plano.badge && (
                  <div style={{
                    ...
                    boxShadow: `0 4px 14px ${plano.cor}44`,
                    whiteSpace: 'nowrap',
                  }}>
                    {plano.badge}
                  </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 7, marginBottom: 16 }}>
                  {plano.items.map((item, i) => (
                    <div key={i} style={{ color: '#cbd5e1', fontSize: 13, display: 'flex', alignItems: 'flex-start', gap: 6 }}>
                      <span style={{ flexShrink: 0, marginTop: 1 }}>{item.slice(0, 2)}</span>
                ...
                {!podeConsultar && (
                  <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.15)', borderRadius: 10, padding: '8px 12px', marginBottom: 12, textAlign: 'center' }}>
                    <span style={{ color: '#f87171', fontSize: 12, fontWeight: 600 }}>
                      ⚠️ Créditos insuficientes — você tem {credits} de {plano.creditos} necessários
                    </span>
                  </div>
                )}
                ...
                <button
                  ...
                  style={{
                    width: '100%',
                    background: podeConsultar
                      ? `linear-gradient(135deg, ${plano.cor}, ${plano.cor}cc)`
                ...
                >
                  {podeConsultar
                    ? `${plano.emoji} Consultar com ${plano.creditos} créditos`
                    : `💳 Comprar créditos`
                  }
                </button>
              </div>
        ...
        <div style={{
          ...
          <div style={{ textAlign: 'center' }}>
            <div style={{ color: '#475569', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Consultas SMART disponíveis</div>
            <div style={{ color: '#3b82f6', fontWeight: 800, fontSize: 22 }}>{Math.floor((credits ?? 0) / 16)}</div>
          </div>
          <div style={{ borderLeft: '1px solid rgba(255,255,255,0.06)' }} />
          <div style={{ textAlign: 'center' }}>
            <div style={{ color: '#475569', fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 4 }}>Consultas Completas disponíveis</div>
            <div style={{ color: '#f59e0b', fontWeight: 800, fontSize: 22 }}>{Math.floor((credits ?? 0) / 48)}</div>
          </div>
          <div style={{ borderLeft: '1px solid rgba(255,255,255,0.06)' }} />
          <div style={{ textAlign: 'center' }}>
